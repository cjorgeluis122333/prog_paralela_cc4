//
//  Dado un vector A de tamaño N.(N no es divisible por la cant de procesos)
//  Cada proceso eleva al cuadrado sus elementos. El proceso 0 imprime el resultado
//Usar: ScaterV y GatherV//
...
int main(int argc, char *argv[]) {
    ...
    if (total_numeros % size == 0) {
        printf("Esta aplicación el numero de procesos no puede ser divisible por la cantidad de procesos.\n");
    ...
    for (int i = 0; i < mi_cantidad; i++) {
        mi_parte[i] = mi_parte[i] * mi_parte[i];
        printf("%d ", mi_parte[i]);
    }
    printf("\n");
