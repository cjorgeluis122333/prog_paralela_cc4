...
/*programa en paralelo que responde al inciso a*/
...
int main(int argc, char *argv[]) {
    ...
    if (local_rows > 0) {
        ...
        for (fila = 0; fila < local_rows; fila++) {
            printf("Fila numero %d ", fila + offsets[rank] / tamanomatriz);
        }
    }
    ...
    // Reducir los máximos locales a un máximo global en el proceso raíz (inciso a)
    ...
    if (rank == 0) {
        printf("\nEl máximo valor de la matriz es: %.1f \n", global_max);
        printf("\nTiempo de ejecución en paralelo %.6f segundos\n", tiempoparalelo);
