#include <stdio.h>
#include <stdlib.h>
#include <stdarg.h>  // Incluido para va_start, va_end
#include <mpi.h>
#include <omp.h>
#include <time.h>
...
int main(int argc, char *argv[]) {
    ...
    if (rank == 0) {
        ...
        for (int dim_idx = 0; dim_idx < num_dim; dim_idx++) {
            ...
            double ts_local = 0.0;  // Declarar ts_local aquí
            ...
            if (rank == 0) {
                ...
                t_fin = MPI_Wtime();
                ts_local = t_fin - t_inicio;  // Asignar valor a ts_local
            ...
            MPI_Bcast(&ts_local, 1, MPI_DOUBLE, 0, MPI_COMM_WORLD);
            ...
            for (int config_idx = 0; config_idx < num_configs; config_idx++) {
                ...
                if (rank == 0) {
                    ...
                    double speedup = ts_local / tp_local;  // CORREGIDO: usar ts_local
        ...
        for (int dim_idx = 0; dim_idx < num_dim; dim_idx++) {
            int dim_filas = dimensiones_filas[dim_idx];
            double ts_local = 0.0;  // Declarar ts_local para procesos no-0

            // Recibir el tiempo secuencial
            MPI_Bcast(&ts_local, 1, MPI_DOUBLE, 0, MPI_COMM_WORLD);
